...
function ENT:Initialize()
	...
	if (SERVER) then
		self.Entity:PhysicsInitBox( Vector() * -4, Vector() * 4 )
	end
...
function ENT:Draw()
	...
	local NumModelPhysBones = self.Entity:GetModelPhysBoneCount()
	if (NumModelPhysBones > 1) then
	
		if ( !self.Entity:GetNetworkedVector( "Vector0", false ) ) then
			return
		end
