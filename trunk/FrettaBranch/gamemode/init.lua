...
concommand.Add("~ProspectiveProfitString",function(ply,cmd,arg) 
	if ply == GAMEMODE.Curator then
		local ent = ents.GetByIndex(arg[1])
		if ent and ent:IsValid() and ent.Item then
			local min = (ent.Item:GetFamilyHappiness()*GAMEMODE.MinHap1)+(ent.Item:GetEnthusistHappiness()*GAMEMODE.MinHap2)+(ent.Item:GetCollectorHappiness()*GAMEMODE.MinHap3)
			local max = (ent.Item:GetFamilyHappiness()*GAMEMODE.MaxHap1)+(ent.Item:GetEnthusistHappiness()*GAMEMODE.MaxHap2)+(ent.Item:GetCollectorHappiness()*GAMEMODE.MaxHap3)
			ply:SendUM("PossibleProfit",ent:EntIndex(),min,max)
		end
	end
end)
...
function GM:Payday()
	self.Curator:SetNWInt("money",self.Curator:GetNWInt("money")+(self.Curator:GetNWInt("happ1")*math.random(self.MinHap1,self.MaxHap1)+self.Curator:GetNWInt("happ2")*math.random(self.MinHap2,self.MaxHap2)+self.Curator:GetNWInt("happ3")*math.random(self.MinHap3,self.MaxHap3))) -- that makes a max of 2500+5000+9000, or 16500. If you're getting this much, your thieves suck, and you pwn.
	local liquid = 0
...
concommand.Add("CuratorMoveDone",function(ply,cmd,args)
	...
	if ply == GAMEMODE.Curator and ent and ent:IsValid() and string.find(ent:GetClass(),"curator_") then
		if pos:IsInMuseum() then
			...
			trace.mask = MASK_SOLID_BRUSHONLY
			local tr = util.TraceLine(trace)
			if (tr.Hit and tr.HitWorld) or ent.IType == "Security" then
				...
				ply:ChatPrint("The object will be moved in 7 seconds.")
			else
				ply:ChatPrint("You can't move that object so high off the ground! Now that would just be unsporting!")
			end
		else
			ply:ChatPrint("You cannot move that outside the museum!")
		end
	else
